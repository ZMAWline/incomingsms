...
test('missing or invalid portMdn blocks a port-in row', () => {
  const missing = validateActivationSim({ iccid: '89014103271467425631', imei: '123456789012345', reseller_id: '1', vendor: 'atomic', port_in: 'true' });
  assert.equal(missing.ok, false);
  assert.match(missing.errors.join('\n'), /port_mdn is required/);

  const invalid = validateActivationSim({ iccid: '89014103271467425631', imei: '123456789012345', reseller_id: '1', vendor: 'atomic', port_in: 'true', port_mdn: '555' });
...
test('CSV template includes headers and new-number plus port-in examples', () => {
  ...
  assert.equal(lines[0], 'iccid,imei,reseller_id,vendor,port_in,port_mdn');
  assert.match(lines[1], /atomic,false,$/);
  assert.match(lines[2], /atomic,true,2125550199$/);
});

test('bulk CSV accepts mixed rows and returns row-level errors', () => {
  const csv = [
    'iccid,imei,reseller_id,vendor,port_in,port_mdn',
    '89014103271467425631,123456789012345,1,atomic,false,',
    '89014103271467425632,123456789012346,1,atomic,true,212-555-0199',
    '89014103271467425633,123456789012347,1,atomic,true,',
  ].join('\n');
