...
test('12h cron + manual run endpoint exist and share the sweep implementation', () => {
  ...
  // The cron enqueues a durable full-fleet job (dedupes if one is already
  // pending) rather than running a fixed-size slice directly; the 1-minute
  // drain below then walks the whole fleet across as many ticks as it takes.
  assert.match(DASHBOARD_SRC, /enqueueHostingPortJob\(env, \{ source: 'cron' \}\)/);
