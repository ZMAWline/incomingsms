...
4. Automatic finalization is implemented in `details-finalizer`: the 5-minute ATOMIC port-in status poll treats `statusCode="00"` + `Result.reasonCode="CO"` as completed, immediately runs `subsriberInquiry` by ICCID through the `mdn-rotator` service binding, and updates the SIM to active with MDN, BAN, IMEI/BLIMEI, activation date/status fields, cleared errors, and `port_in_pending=false`. If the inquiry is transiently unavailable, the SIM stays `port_in_pending=true` so the next poll retries.
5. Terminal statuses stop the poll without touching `sims.status` — `948` (`Error!!Port Request Does Not Exist`), `910` (SIM not on our ATOMIC account), and `951` (`Result.reasonCode="CT"`, losing carrier rejected the port; the real cause is embedded in the description as `statusReasonCode - <XX> ~ statusReasonDescription - <text>`, e.g. `8A` wrong account number, `6B` wrong T-Mobile transfer PIN). A `951` needs the port details corrected and a fresh `portinRequest` — polling will never clear it. Any other code keeps polling.
