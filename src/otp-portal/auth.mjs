// Pure auth helpers for the otp-portal worker: PBKDF2 password hashing/
// verification and HMAC-signed, expiring login-session tokens. No I/O, so
// testable under node:test with no bundling — same shape as
// src/storefront/auth.mjs, kept as a separate copy per this repo's
// convention of not importing across worker directories.
...
// HMAC-signed, expiring login-session tokens: otpps_<b64url(expiresAtMs)>.<sig>
...
export async function signSession(secret, ttlMs, nowMs = Date.now()) {
  ...
  return 'otpps_' + u8ToB64url(new TextEncoder().encode(payload)) + '.' + sig;
}

export async function verifySession(secret, token, nowMs = Date.now()) {
  if (!secret || !token || !token.startsWith('otpps_')) return false;
  const rest = token.slice('otpps_'.length);
