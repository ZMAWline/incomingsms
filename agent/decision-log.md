...
## 2026-08-20 — bulk-activator queue consumer's "already activated" guard now checks status, not just msisdn presence (task t_184b6f38)

**Decision:** `queue()` in `src/bulk-activator/index.js` used to skip any SIM whose `msisdn` or `mobility_subscription_id` column was non-empty, ack'ing the message with only a `console.log` — no DB write, no `carrier_api_logs` row. Now it also requires `status` to be `active` or `provisioning` before treating those columns as meaning "already activated."
...
## 2026-09-25 - One Runs page for activation runs and bulk jobs, joined by a view
