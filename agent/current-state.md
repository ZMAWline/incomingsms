...
> 2026-09-25 (latest): PR #138 (native `quickbooks` Worker OAuth invoice automation) deployed to PROD (dashboard `0dcb3dd6`, quickbooks `94b8d0c9`, both 15:33 UTC; Friday `0 17 * * 5` cron confirmed live via the CF API at 15:34 UTC). Reconciliation/send was requested but BLOCKED: QuickBooks OAuth is disconnected in PROD (`QBO_TOKENS` KV empty; live `/invoice/query` throws `Not connected to QuickBooks`, confirmed via `wrangler tail`). Nothing was created or sent — today's 17:00 UTC cron will fire and fail the same way (caught, logged, no crash). Also: local `qbo_invoices` is proven incomplete — QBO invoice 1203 (week 2026-08-21..08-27, already `EmailSent`/`Viewed`, see kanban `t_81e68aff`) has no local row, so "no local row" cannot be trusted to mean "absent" for any week. See "QuickBooks OAuth disconnected, reconciliation blocked (2026-09-25)".

---

## QuickBooks OAuth disconnected, reconciliation blocked (2026-09-25)

- **Task:** reconcile TrustOTP/HYPPE weekly invoices (sent / unsent existing / absent) and send unambiguous outstanding ones, per owner approval recorded on kanban `t_81e68aff`. Full findings and the reconciliation table are posted as a comment on that task; this is the short version for the next session.
- **Independently verified live versions** (previous handoff on the same kanban card warned not to trust prior claimed status): `quickbooks` worker live version `94b8d0c9-b40c-4f91-a588-00aaf289d588`; `dashboard` live version `0dcb3dd6-a7c3-4f1e-88c6-d17479e384c3`; both deployed 2026-09-25 15:33 UTC (by the prior session, ~10 min before this one started). Friday cron `0 17 * * 5` is registered live on prod `dashboard` (CF API `GET /workers/scripts/dashboard/schedules`, `modified_on` 15:34:00 UTC). The new `quickbooks` routes from PR #138 (`/items/search`, `/invoice/query`, `/invoice/:id/send`, `/invoice/:id`) are live — confirmed by calling them directly.
- **Blocker: QuickBooks OAuth is disconnected in PROD.** `wrangler kv key list --binding=QBO_TOKENS --remote` (from `src/quickbooks/`) returns `[]`. A live `GET /invoice/query` against `https://quickbooks.zalmen-531.workers.dev` returns HTTP 500; `wrangler tail` on the same request shows `Error: Not connected to QuickBooks` thrown from `getValidAccessToken`. This blocks every QBO read and write — reconciliation cannot be checked against live QuickBooks, and nothing can be created or sent. Today's 17:00 UTC Friday cron will fire and fail the same way; `scheduled()` catches and logs it, so it won't crash the worker, but no invoice will be produced.
- **Reconnect path (needs a human with the TrustOTP QuickBooks login):** `GET https://quickbooks.zalmen-531.workers.dev/auth-url` returns a fresh Intuit consent URL; opening it and approving completes the OAuth flow via `redirect_uri=https://dashboard.zalmen-531.workers.dev/api/qbo/callback` and repopulates `QBO_TOKENS`. Not something I should do myself — it's the account owner's Intuit login.
- **Local `qbo_invoices` (mapping id 1, TrustOTP/HYPPE TECH, $1.10/SIM-day) is proven incomplete, not just possibly stale:** week 2026-08-21..08-27 has **no local row**, but per this same kanban card's own incident history, QBO invoice `1203` for that exact week already exists and reads `EmailSent`/`Viewed`. So "no local row" cannot be treated as "absent" for any week without a live `/invoice/query` cross-check first — doing otherwise risks double-invoicing a customer. That 1203 incident itself is still open and unresolved (needs an accounting decision: leave / void / correct), independent of today's work.
- **State of the local table today** (read-only, not touched): weeks 2026-03-06..05-15 (10 weeks) are `paid` with no QBO id — pre-automation/manual, left alone. Weeks 2026-05-16..08-13 (13 weeks) are `draft` with no QBO id — CSV-download placeholders, ambiguous, unverifiable while OAuth is down. 2026-08-14..08-20 has no row (unverifiable). 2026-08-21..08-27 has no row but a real sent invoice exists (1203, above). 2026-08-28..09-03 is `created` (QBO invoice 1205, not marked sent locally — live `EmailStatus` unverifiable right now). 2026-09-04..09-10 is `sent` (QBO invoice 1450) and is correctly protected by the automation's own guard. 2026-09-11..09-17 and 2026-09-18..09-24 have no local row and are the only weeks that look like clean create-candidates, but per the point above they are **not** confirmed absent.
- **Net effect of this session:** 0 invoices created, 0 sent, 0 rows in `qbo_invoices` changed. Only read-only Supabase `SELECT`s and read-only HTTP probes against the deployed `quickbooks` worker were made. **Next session:** once OAuth is reconnected, cross-check every candidate week (starting with 2026-09-11..09-17 and 2026-09-18..09-24) against a live `/invoice/query` by `DocNumber` before creating anything, then resolve invoice 1203's incident state, then let the Friday cron run going forward.

## Open-items sweep (2026-09-25)

- **Google Sheets export shipped and WORKING in PROD (PR #136 `a5528e9`, PR #137 `aa28e3d`).** 554 lines had sat uncommitted in the `export-to-google-sheet` worktree since 2026-09-18. Rebased onto main: the SIMs export row builder now fetches every filtered SIM from `/api/sims` 500 at a time (the #121 server-side paging). Export menu: CSV, TXT, Google Sheets. `POST /api/sims/export/google-sheet` (operator+, API keys allowed) creates the sheet inside the **"Dashboard SIM exports"** folder (`1OR3MQjiiEXPch37BPsEmDHHWKS1pFzF2`) of the **Server Backups** Shared Drive (`0AE85XWlYLJd1Uk9PVA`), which the `dashboard-sheets-export@triple-silo-502413-g7` service account is a member of. Config: `GOOGLE_SERVICE_ACCOUNT_JSON` secret set on PROD `dashboard` from `/root/.config/google/dashboard-sheets-export.json` (dashboard-test already had it); `GOOGLE_SHEETS_PARENT_FOLDER_ID` in `[vars]` and `[env.test.vars]`. Deployed: `ebec9d3d` (#136) then `aa290d39` (#137), 1202/1202 tests each, root 200. **Smoke test:** a POST with the PROD agent API key returned 200 with a spreadsheet URL; the test sheet was deleted afterwards. Worktree and branches removed. `agent/secrets-inventory.md`: the secret is no longer a delete candidate (42 → 41).
- **#84 invoice filename gap — already closed.** `invoiceDownloadName()` in `public/index.html` reads `Content-Disposition` for both invoice downloads; it landed in #127 (`cd50e29`) and shipped in ship 8. The 2026-09-22 note above is stale.
- **Unexplained redeploys — SOLVED: Cloudflare Workers Builds (Git integration).** Every push to `main` is followed within ~40 s by a `script_update` + `script_deploy` of exactly four workers — **mdn-rotator, details-finalizer, bulk-activator, reseller-sync** — recorded in the Cloudflare audit log with actor type `user`, email `zalmen@zmawsolutions.com`, and rotating Cloudflare-owned IPs (`104.28.x.x`), while deploys from this server via `scripts/deploy.sh` show actor type `token` and IP `82.38.44.154`. Confirmed live today: push at 14:13 UTC (PR #136 merge) → the four deployed 14:14:04–14:14:12; push at 14:16:01 → 14:16:43–14:16:51; also 09-25 12:33 (rotation-review commit), 09-24 16:15/16:29, 09-23 03:07/03:15. Branch pushes produce `script_update` only, with a `workers/alias` of the branch name (preview uploads). So those four workers are connected to the GitHub repo in the Cloudflare dashboard, and Cloudflare builds and deploys them on every push, under the owner's identity. This bypasses `scripts/deploy.sh` (tests, DB constraint check, live-var guard) and deploys even doc-only commits. The API token here cannot read the build config (`/builds/workers/*/config` → Invalid token). **DISCONNECTED 2026-09-25 ~14:40 UTC:** the owner created a user-scoped token (`~/.config/cloudflare/builds-token`, Workers Builds Configuration: Edit + Workers Scripts: Read; the account token `cfat_…` is refused by the Builds API by design) and both triggers ("Deploy default branch" → `npx wrangler deploy`, "Deploy non-production branches" → `npx wrangler versions upload`, all created 2026-01-19) were deleted with `DELETE /builds/triggers/{uuid}` on **five** workers: mdn-rotator, details-finalizer, bulk-activator, reseller-sync and sms-ingest (sms-ingest had the same triggers but never showed in the audit log). `GET /builds/workers/{tag}/triggers` now returns 0 on all five. `imei-generator` (a different repo, `Zsimcall/imei-generator`, created 2026-03-27) keeps its triggers. From now on `scripts/deploy.sh` from the main checkout is the only PROD deploy path; nothing has been reverted by the builds, they always deployed `main`.
- **Cloud routine `rotation-review` (trig_017nq9h7VPDnfoSy6dLhRnCR)** runs daily 12:30 UTC on claude-sonnet-4-6 with the Google Drive and Cloudflare connectors attached, and its prompt embeds the details-finalizer and mdn-rotator admin secrets in plain text. Worth moving those to a connector or rotating them; not changed today.
- **Deploy guard (2026-09-18, `feb91d7`), never recorded here:** `scripts/deploy.sh` refuses a PRODUCTION deploy when `HEAD` is not `main` or `main` is behind `origin/main`; warns (does not block) on uncommitted `src/` changes; preview deploys unaffected; override `ALLOW_UNSAFE_DEPLOY=1`. `agent/BOOTSTRAP.md` rule 8 says build in worktrees, deploy from an up-to-date main. The stale `chore/record-deploy-guard` worktree and branch that held this note were removed.
- **CLOSED, owner decision 2026-09-25: keep the 41 unused live secrets.** Do not delete them; they stay for a possible future re-enable of the legacy vendors. The "Live secrets no code reads" list in `agent/secrets-inventory.md` is an inventory, not a to-do. The 105 secrets TEST lacks remain a separate, open decision.
- **Still open, skipped by owner request today:** rotation-review items (20 stuck atomic SIMs since 2026-09-09, 21 open bad rental reports, 200 pending operator items).
