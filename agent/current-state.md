...
> Last updated: 2026-04-24 evening (session 35 — provisioning+finalizer pattern extended to Wing IoT verify-plan, Teltik, ATOMIC 5xx safety net)
...
- **198 Wing IoT SIMs stuck on ABIR (non-dialable) plan as of 2026-04-24 evening** — AT&T accepted the second PUT (dialable) with `202 Accepted` but never committed the plan change. `dateUpdated` on AT&T side stayed at morning rotation time even after fresh PUT attempts in the afternoon. Suspected daily per-SIM plan-change quota at AT&T. DB `msisdn` matches AT&T's current (non-dialable) MDN, so no internal state drift; but those 198 SIMs have non-geographic MDNs (500/533x prefixes) that can't receive normal calls/SMS. Remediation endpoint exists (`POST mdn-rotator/remediate-stuck-wing`); wait until tomorrow (fresh quota) and re-run. ICCID list available from DB: `WHERE vendor='wing_iot' AND LEFT(msisdn,3) IN ('500','533','534','535','536','537','538','539')`.
