...
- **4 suspended ATOMIC SIMs (IDs 984, 735, 990, 993)** — `attStatus: "Suspended"`, SOC `DSABR2`. swapMSISDN returns "Subscriber Must Be Active"; cron keeps failing → `rotation_fail_count` = 2 at session close. On the next cron tick they hit 3 and flip to `status='rotation_failed'`. Fix: POST to `/fix-sim` with `{"sim_ids": [984, 735, 990, 993]}` — `fixAtomicSim` will restore them. Do this before the next rotation cron fires.
