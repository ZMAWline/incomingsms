...
> 2026-09-25: Open-items sweep. PR #136 (SIMs Export → Google Sheets, recovered from the 2026-09-18 worktree) merged and DEPLOYED to PROD — dashboard `ebec9d3d`. `deployed/prod` 3e264a3 → a5528e9. The #84 invoice filename gap was already fixed by #127. The unexplained redeploys come from a wrangler OAuth login outside this server. See "Open-items sweep (2026-09-25)".

---

## Open-items sweep (2026-09-25)

- **Google Sheets export shipped (PR #136, `a5528e9`).** 554 lines had sat uncommitted in the `export-to-google-sheet` worktree since 2026-09-18. Rebased onto main: the SIMs export row builder now fetches every filtered SIM from `/api/sims` 500 at a time (the #121 server-side paging) instead of the old client-side `getFilteredSims()`. Export menu: CSV, TXT, Google Sheets. `POST /api/sims/export/google-sheet` (operator+) answers 501 until both `GOOGLE_SERVICE_ACCOUNT_JSON` (secret) and `GOOGLE_SHEETS_PARENT_FOLDER_ID` (`[vars]`, a Shared Drive folder the service account is a Content manager of) exist. **Today:** PROD `dashboard` has neither; `dashboard-test` has the secret but not the folder var. So the button shows "not configured yet" everywhere until the owner supplies a Shared Drive folder ID and the PROD key (`src/dashboard/wrangler.toml` has the exact commands). Deployed via `scripts/deploy.sh dashboard --env=""`: 1202/1202 tests, DB check pass, version `ebec9d3d-073f-4b41-a568-26444488169b`, root 200 → 200, live bundle contains `handleSimsExportGoogleSheet`. Worktree and branch removed. `agent/secrets-inventory.md`: the secret is no longer a delete candidate (42 → 41).
...
- **Unexplained redeploys — traced as far as this server allows.** Every version this server deploys with `scripts/deploy.sh` (API token from `~/.config/cloudflare/env`) shows an EMPTY `author_email` in the Cloudflare versions API. The unexplained versions (mdn-rotator / details-finalizer / bulk-activator on 2026-09-24 16:15 and 16:29 UTC, 2026-09-25 12:33 and 14:04 UTC) all carry `author_email = zalmen@zmawsolutions.com`, i.e. a `wrangler login` OAuth identity. This server has no OAuth login (`wrangler whoami` without the token: "not authenticated"). No local Claude transcript, hook, crontab, systemd timer, GitHub Actions run, or `.git` hook ran a deploy in those windows, and the cloud `rotation-review` routine's run log (session `cse_01Mo97exYUQL7o5y3MdQcEsL`) shows only curl + git; its prompt forbids deploys. The 14:04:57 UTC batch started 7 s after the owner opened a session through the Orca desktop relay (10:04:43 EDT). **Conclusion:** another machine, logged in to wrangler as the owner, deploys these three workers — most likely a checkout on the owner's own computer with a startup or sync step. Nothing was reverted (live code still has the `LEGACY_VENDORS` gate). **Owner action:** on that computer run `npx wrangler whoami` and look for a repo checkout or app task that runs `wrangler deploy` on launch.
