...
### QuickBooks OAuth reconnection pending owner approval (2026-09-25)
Composio was confirmed to have no working QBO path (see Recent Significant Changes below) so the native flow is the only option. The `quickbooks` worker's `/auth-url` endpoint returned a valid Intuit consent URL; **the account owner (Zalmen) still needs to open it and approve** to actually reconnect QBO. Until that happens, invoice creation/sending through `quickbooks` stays unauthorized (existing tokens in `QBO_TOKENS` KV are presumably the ones that broke — cause of the original break was not diagnosed this session, only the reconnection path).
...
|------|--------|-----------|
| 2026-09-25 | **QuickBooks connection recovery — Composio confirmed dead, native OAuth link obtained (read-only session, no code changes).** Investigated per owner request whether Composio could provide a working QBO read before falling back to native OAuth. Confirmed no Composio integration exists anywhere in the repo: `COMPOSIO_API_KEY` (dashboard secret, per `agent/secrets-inventory.md`) has no code reading it, no Composio SDK/dependency in `package.json`, no Composio MCP server connected — nothing to invoke. Fell back to native path: `GET https://quickbooks.zalmen-531.workers.dev/auth-url` (public, no auth, no state mutated) returned a valid Intuit consent URL (`appcenter.intuit.com/connect/oauth2`, correct client_id/scope/redirect_uri/state). URL was handed to the owner to complete the OAuth approval manually — not followed/approved by the agent. QBO remains unauthorized until the owner completes that consent flow; `/status`, `/callback`, and token endpoints were deliberately not touched this session. | quickbooks (read-only) |
